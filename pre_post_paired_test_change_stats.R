...
pre_post_overall_paired_change_stats <- sapply(outcome_vars, function(x){
  nn <- x
  ...
  df <- dplyr::bind_rows(df_drop_vars %>% dplyr::mutate(data_type = "Raw Data"),
   ...
   table_out <- tbl_strata(df_new,
                           ...
                           .tbl_fun = ~ .x %>%
                             ...
                             add_difference(test = list(any_of(outcome_vars) ~ "paired.t.test"),
...
pre_post_overall_paired_change_stats_merge <- tbl_stack(pre_post_overall_paired_change_stats,
                                                 group_header = attribute$label[attribute$variable %in% outcome_vars]
                                                 ) %>%
  gtsummary::as_flex_table()


pre_post_gender_paired_change_stats <- sapply(unique(as.character(df_drop_vars$wdf_q1_5)), function(y){
  gg <- y
